...
function App() {
  ...
  async function postLoop () {
    ...
    let playlist_data = [8] // col 0 is track title, following 3 are artists on the track --> each row is a new track
    let twodplaylist_data = [10][10]
    ...
    for(let i = 0; i < playlist_details.total; i++) {
      // console.log(playlist_details.items[i].track.name)
      playlist_data[i] = playlist_details.items[i].track.name;
      // twodplaylist_data[i][i] = playlist_details.items[i].track.name;
    }
    console.log(playlist_data)
    // console.log(twodplaylist_data)
    ...
    try {
      ...
      while (true && trackIndex < 10) { // need to come up with a solution to too few results
        ...
        if (currTrack.name.charAt(0).toLowerCase() === searchChar ) { // include genre right here
          ...
          if(!validateTrack(`spotify:track:${validTrackID}`)) { // feels wrong
            trackIndex++;
            continue;
          }
          break;
  ...
  function validateTrack ( trackID ) {
    ...
    while (index < finalTracks.length) {
      console.log("trackID: " + trackID + " and finalTracks[index]: " + finalTracks[index] + " evaluates to: ")
      console.log(trackID === finalTracks[index])
